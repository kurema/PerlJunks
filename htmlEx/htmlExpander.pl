...
sub getLocalTimeHash{
	...
	$TIME{"wDayJapanese"}=qw(ì˙ åé âŒ êÖ ñÿ ã‡ ìy)[$TIME{"wDay"}];
	...
	$TIME{"mDayJapanese"}=qw(ñråé î@åé ñÌê∂ âKåé éHåé êÖñ≥åé ï∂åé ótåé í∑åé ê_ñ≥åé ëöåé ê_äyåé étëñ)[$TIME{"month"}];
	...
	$TIME{"textDateJapanese"}=sprintf("%04d\îN%02d\åé%02d\ì˙(%s\)",$TIME{"year"},$TIME{"month"},$TIME{"mDay"},$TIME{"wDayJapanese"});
	$TIME{"textTimeJapanese"}=sprintf("%02d\éû%02dï™%02dïb",$TIME{"hour"},$TIME{"min"},$TIME{"sec"});
